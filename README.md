...
- **发布日历与即将上映**：即将上线内容预览和跟踪，支持收藏即将上映内容，上映后自动可播放
 - **个性化为你推荐**：结合继续观看与收藏，按权重（收藏/近期/未看完/时间衰减）计算综合分，首页新增「为你推荐」模块，滑杆可调参数
 - **最近热播（聚合）**：支持将热门区块聚合为「最近热播」条目，后续可接入全站播放统计作为热度来源
...
- **选集分组滚动翻页**：播放页选集支持滚动翻页，大量集数流畅浏览
 - **一起看（Watch Party）**：房间号与邀请链接、成员列表与聊天、加入时自动对齐主机当前进度、跟随主机开关；移动端聊天默认折叠；采用 SSE 广播保证实时同步
...
- **虚拟滚动**：react-window 2.2.0，支持大量内容流畅加载，智能容器尺寸检测（ResizeObserver）
 - **液态玻璃按钮**：横幅切换与滚动箭头采用「Liquid Glass」玻璃态视觉，质感统一
